...
public class CorefChain implements Serializable {
  ...
  public CorefChain(CorefCluster c, Map<String, IntTuple> positions){
    ...
    c.getCorefMentions().stream().map((m) -> new CorefMention(positions.get(m))).forEach((men) -> mentions.add(men));
    ...
    for (CorefMention men : mentions) {
      ...
      if (!mentionMap.containsKey(position)) mentionMap.put(position, new HashSet<>());
