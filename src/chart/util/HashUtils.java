...
import chart.util.BooleanList;
import chart.util.StrokeList;
...
public class HashUtils {
	...
	/**
	 * Returns a hash code for a <code>Paint</code> instance. If <code>p</code> is
	 ...
	 * JCommon, the {@link BooleanList} class should implement the hashCode() method
	 * correctly, but we compute it here anyway so that we can work with older
	 * versions of JCommon (back to 1.0.0).
	 * 
	 * @param pre  the seed value.
	...
	public static int hashCode(int pre, BooleanList list) {
		...
		if (size > 0) {
			...
			if (size > 1) {
				result = HashUtils.hashCode(result, list.getBoolean(size - 1));
				if (size > 2) {
					result = HashUtils.hashCode(result, list.getBoolean(size / 2));
				}
			}
		}
	 ...
	 * JCommon, the {@link StrokeList} class should implement the hashCode() method
	 * correctly, but we compute it here anyway so that we can work with older
	 * versions of JCommon (back to 1.0.0).
	 * 
	 * @param pre  the seed value.
	...
	public static int hashCode(int pre, StrokeList list) {
		...
		if (size > 0) {
			...
			if (size > 1) {
				result = HashUtils.hashCode(result, list.getStroke(size - 1));
				if (size > 2) {
					result = HashUtils.hashCode(result, list.getStroke(size / 2));
				}
			}
		}
		return 37 * pre + result;
