...
public class SerialUtils {
	...
	/**
	 * Serialises a {@code Stroke} object. This code handles the {@code BasicStroke}
	 * class which is the only {@code Stroke} implementation provided by the JDK
	 * (and isn't directly {@code Serializable}).
	 *
	...
	public static AttributedString readAttributedString(ObjectInputStream stream)
		...
		if (!isNull) {
			...
			while (c != CharacterIterator.DONE) {
				...
				Map<? extends AttributedCharacterIterator.Attribute, ?> atts = (Map<? extends AttributedCharacterIterator.Attribute, ?>) stream
						.readObject();
	...
	public static void writeAttributedString(AttributedString as, ObjectOutputStream stream) throws IOException {
		...
		if (as != null) {
			...
			while (current != CharacterIterator.DONE) {
				...
				Map<AttributedCharacterIterator.Attribute, Object> atts = new HashMap<>(aci.getAttributes());
	 ...
	 * @throws ClassNotFoundException if there is a problem loading a class.
	 */
	public static Map<Integer, Paint> readPaintMap(ObjectInputStream in) throws IOException, ClassNotFoundException {
	 ...
	 * @throws IOException if there is an IO problem.
	 */
	public static void writePaintMap(Map<Integer, Paint> map, ObjectOutputStream out) throws IOException {
