...
public class PlotRenderingInfo implements Cloneable, Serializable {
	...
	/** The plot area. */
	private transient Rectangle2D plot_Area;

	/** The data area. */
	private transient Rectangle2D data_Area;

	/**
	 * Storage for the plot rendering info objects belonging to the subplots.
	 */
	private List<PlotRenderingInfo> subplot_Info;
	...
	public PlotRenderingInfo(ChartRenderingInfo owner) {
		this.owner = owner;
		this.data_Area = new Rectangle2D.Double();
		this.subplot_Info = new java.util.ArrayList<>();
	}
	 ...
	 * Sets the plot area.
	 *
	 * @param area the plot area (in Java2D space, <code>null</code> permitted but
	 *             discouraged)
	 *
	 * @see #getPlotArea()
	 */
	 ...
	 * Sets the data area.
	 *
	 * @param area the data area (in Java2D space, <code>null</code> permitted but
	 *             discouraged).
	 *
	 * @see #getDataArea()
	 */
	...
	public PlotRenderingInfo getSubplotInfo(int index) {
		return this.subplot_Info.get(index);
	}
	...
	public boolean equals(Object obj) {
		...
		if (!ObjectUtils.equal(this.data_Area, that.data_Area)) {
			return false;
		}
		if (!ObjectUtils.equal(this.plot_Area, that.plot_Area)) {
			return false;
		}
		if (!ObjectUtils.equal(this.subplot_Info, that.subplot_Info)) {
			return false;
		}
		return true;
	}
	...
	public Object clone() throws CloneNotSupportedException {
		...
		if (this.plot_Area != null) {
			clone.plot_Area = (Rectangle2D) this.plot_Area.clone();
		}
		if (this.data_Area != null) {
			clone.data_Area = (Rectangle2D) this.data_Area.clone();
		}
		clone.subplot_Info = new java.util.ArrayList<>(this.subplot_Info.size());
		for (int i = 0; i < this.subplot_Info.size(); i++) {
			PlotRenderingInfo info = this.subplot_Info.get(i);
			clone.subplot_Info.add((PlotRenderingInfo) info.clone());
		}
		return clone;
	}
	...
	private void writeObject(ObjectOutputStream stream) throws IOException {
		...
		SerialUtils.writeShape(this.data_Area, stream);
		SerialUtils.writeShape(this.plot_Area, stream);
	}
	...
	private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException {
		...
		this.data_Area = (Rectangle2D) SerialUtils.readShape(stream);
		this.plot_Area = (Rectangle2D) SerialUtils.readShape(stream);
	}
