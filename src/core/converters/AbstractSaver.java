...
import core.CapabilitiesIgnorer;
...
public abstract class AbstractSaver implements Saver, CapabilitiesHandler, CapabilitiesIgnorer {
	...
	public int setStructure(Instances headerInfo) {
		...
		if (mWriteMode == WAIT && headerInfo != null) {
			mInstances = headerInfo;
			mWriteMode = STRUCTURE_READY;
		} else {
			if ((headerInfo == null) || !(mWriteMode == STRUCTURE_READY) || !headerInfo.equalHeaders(mInstances)) {
				mInstances = null;
				if (mWriteMode != WAIT) {
